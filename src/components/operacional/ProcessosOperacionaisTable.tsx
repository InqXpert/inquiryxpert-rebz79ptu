...
export function ProcessosOperacionaisTable({ processos, loading, onViewDetail, pagination, setPagination }: Props) {
  ...
  if (loading) {
    return <div className="space-y-4">{[1, 2, 3].map(i => <div key={i} className="h-16 bg-muted animate-pulse rounded-lg" />)}</div>
  }
        ...
        {pageData.map(p => (
          <Card key={p.id} className="p-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
              <div><span className="block font-medium">Seguradora</span>{p.cia}</div>
              <div><span className="block font-medium">Data Entrada</span>{p.data_entrada}</div>
            </div>
            <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => onViewDetail(p.id)}>
              Ver Detalhes
            </Button>
          </Card>
        ))}
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              <TableHead className="cursor-pointer" onClick={() => handleSort('numero_controle')}>Controle</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('status')}>Status</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('nome_segurado')}>Segurado</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('cia')}>Seguradora</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('tipo_servico')}>Serviço</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('data_entrada')}>Entrada</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort('resultado')}>Resultado</TableHead>
              <TableHead className="text-right">Ações</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {pageData.map((p, i) => (
              <TableRow key={p.id} className="hover:bg-muted/50 transition-colors animate-in slide-in-from-bottom-2 fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                <TableCell className="font-medium text-xs text-muted-foreground">{p.numero_controle}</TableCell>
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="sm" onClick={() => onViewDetail(p.id)} aria-label="Detalhes">Detalhes</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = {
    em_elaboracao: 'Em Elaboração', em_execucao: 'Em Execução',
    finalizado: 'Finalizado', cancelado: 'Cancelado', analise_inicial: 'Análise Inicial'
  }
  return (
    <span role="status" aria-label={`Status: ${labels[status] || status}`} className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold border ${colors[status] || 'bg-muted text-muted-foreground'}`}>
...
function ResultadoBadge({ resultado }: { resultado: string }) {
  ...
  return (
    <span role="status" className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold border capitalize ${colors[resultado] || 'bg-muted text-muted-foreground'}`}>
      {resultado}
    </span>
  )
}
