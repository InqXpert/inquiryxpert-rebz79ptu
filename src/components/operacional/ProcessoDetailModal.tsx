...
export function ProcessoDetailModal({ processoId, isOpen, onClose, onUpdated }: Props) {
  const { 
    processo, historico, documentos, loading, fetchProcessoDetail,
          ...
          <>
            <DialogHeader className="p-6 pb-4 border-b shrink-0 bg-background/95 backdrop-blur z-10">
              <div className="flex items-center justify-between gap-4">
                ...
                {canDeleteProcesso() && (
                  <Button variant="destructive" size="sm" onClick={() => {}}>Excluir Processo</Button>
                )}
              </div>
            </DialogHeader>
            ...
            <div className="flex-1 overflow-y-auto bg-muted/10 p-6 pt-2">
              <Tabs defaultValue="gerais" className="w-full h-full flex flex-col">
                <TabsList className="w-full justify-start border-b rounded-none bg-transparent h-12 p-0 space-x-6">
                  <TabsTrigger value="gerais" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-1 py-3 text-sm">Inf. Gerais</TabsTrigger>
                  <TabsTrigger value="posicoes" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-1 py-3 text-sm">Posições</TabsTrigger>
                  <TabsTrigger value="observacoes" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-1 py-3 text-sm">Observações</TabsTrigger>
                  <TabsTrigger value="documentos" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-1 py-3 text-sm">Documentos</TabsTrigger>
                  <TabsTrigger value="historico" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-1 py-3 text-sm">Histórico</TabsTrigger>
                </TabsList>
                
                <div className="mt-4 flex-1">
                  <TabsContent value="gerais" className="m-0 h-full">
                    <TabInformacoesGerais processo={processo} canEdit={canEditProcesso()} onSave={(d) => { updateProcesso(d); onUpdated(); }} />
                  </TabsContent>
                  ...
                  <TabsContent value="observacoes" className="m-0 h-full">
                    <TabObservacoes processo={processo} canAdd={canAddObservacao()} onAdd={addObservacao} />
                  </TabsContent>
                  <TabsContent value="documentos" className="m-0 h-full">
                    <TabDocumentos documentos={documentos} canUpload={canUploadDocumento()} onUpload={uploadDocumento} onDelete={deleteDocumento} />
                  </TabsContent>
