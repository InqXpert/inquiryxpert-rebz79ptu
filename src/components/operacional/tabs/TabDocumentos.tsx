  ...
  return (
    <div className="space-y-6 pt-4 animate-in fade-in">
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-4">Arquivos Anexados ({documentos.length})</h4>
        {documentos.length === 0 ? (
          <p className="text-sm text-muted-foreground italic text-center py-4">Nenhum documento anexado.</p>
        ) : (
          documentos.map(doc => (
            <div key={doc.id} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{(doc.size / 1024).toFixed(1)} KB • {new Date(doc.created).toLocaleDateString()}</p>
                </div>
              </div>
