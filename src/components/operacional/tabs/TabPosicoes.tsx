        ...
        {posicoes.map(p => (
          <div key={p.n} className="relative pl-6">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold text-muted-foreground uppercase">{p.label}</span>
              {editingPos === p.n ? (
                <div className="flex flex-col gap-2 w-full max-w-md">
                  <Input value={text} onChange={(e) => setText(e.target.value)} placeholder="Descreva a posição..." />
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => handleSave(p.n)}>Salvar</Button>
                    <Button size="sm" variant="ghost" onClick={() => setEditingPos(null)}>Cancelar</Button>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col gap-2 items-start">
                  <p className="text-sm text-foreground bg-muted/50 p-3 rounded-md w-full max-w-md border">
                    {p.val || <span className="text-muted-foreground italic">Aguardando preenchimento...</span>}
                  </p>
                  {canAdd && (
                    <Button variant="link" size="sm" className="h-auto p-0 text-xs text-primary" onClick={() => handleEdit(p.n, p.val)}>
