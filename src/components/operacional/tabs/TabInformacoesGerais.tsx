...
export function TabInformacoesGerais({ processo, canEdit, onSave }: Props) {
  ...
  const fields = [
    ...
    { key: 'status', label: 'Status', type: 'select', options: ['em_elaboracao', 'em_execucao', 'finalizado', 'cancelado', 'analise_inicial'] },
    ...
    { key: 'resultado', label: 'Resultado', type: 'select', options: ['regular', 'irregular', 'analise', 'cancelado'] },
  ...
  return (
    <div className="space-y-6 pt-4 animate-in fade-in">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-semibold text-foreground uppercase tracking-wide">Dados Cadastrais</h3>
        {canEdit && !isEditing && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        {fields.map(f => (
          <div key={f.key} className="flex flex-col gap-1.5">
            ...
            {isEditing ? (
              f.type === 'select' ? (
                <Select value={(formData as any)[f.key] || ''} onValueChange={(v) => handleChange(f.key as any, v)}>
                  <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {f.options?.map(opt => <SelectItem key={opt} value={opt} className="capitalize">{opt.replace('_', ' ')}</SelectItem>)}
                  </SelectContent>
                </Select>
              ) : (
                <Input 
              ...
              <span className="text-sm font-medium text-foreground capitalize">
                {(processo as any)[f.key] ? String((processo as any)[f.key]).replace('_', ' ') : '-'}
              </span>
            )}
          </div>
        ))}
      ...
      {isEditing && (
        <div className="flex justify-end gap-3 pt-4 border-t mt-6">
          <Button variant="ghost" onClick={() => { setIsEditing(false); setFormData(processo) }}>Cancelar</Button>
