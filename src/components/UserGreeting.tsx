...
export const UserGreeting = memo(function UserGreeting() {
    ...
    return (
      <div className="flex flex-row items-center gap-4 sm:gap-6 bg-card rounded-lg p-6 shadow-sm mb-6 border-none">
        ...
        <div className="flex flex-col items-start justify-center gap-2 w-full">
          ...
          <Skeleton className="h-8 w-24 mt-1 sm:mt-2" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-row items-center gap-4 sm:gap-6 bg-card rounded-lg p-6 shadow-sm mb-6 border-none">
      ...
      >
        <Avatar className="w-full h-full border-none outline-none ring-0">
          <AvatarImage
            src={localAvatarUrl || undefined}
            alt={fullName}
            className="object-cover border-none outline-none ring-0"
          />
          <AvatarFallback className="bg-muted text-muted-foreground text-xl sm:text-2xl font-bold uppercase border-none outline-none ring-0">
            {initials}
          </AvatarFallback>
        </Avatar>

        {/* Overlay on hover */}
        <div className="absolute inset-0 bg-black/40 text-white opacity-0 group-hover:opacity-100 rounded-full flex flex-col items-center justify-center transition-opacity duration-200 border-none outline-none ring-0">
      ...
      <div className="flex flex-col items-start justify-center gap-1 flex-1">
        <h1 className="font-semibold text-foreground text-left tracking-tight text-lg md:text-xl">
          {saudacao}, <span className="font-bold uppercase">{firstName}</span>
        </h1>

        <p className="text-muted-foreground text-xs md:text-sm text-left font-medium m-0">
          {formattedDate}
        </p>

        <div className="font-bold text-foreground tracking-tight text-2xl sm:text-3xl md:text-4xl mt-1 sm:mt-2">
          {hours}:{minutes}
        </div>
      </div>
    </div>
