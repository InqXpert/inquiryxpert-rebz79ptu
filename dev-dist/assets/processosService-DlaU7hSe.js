...
var searchProcessos = (processos, search) => {
	...
	return processos.filter((p) => (p.id || "").toLowerCase().includes(lower) || (p.numero_controle || "").toLowerCase().includes(lower) || (p.data_entrada || "").toLowerCase().includes(lower) || (p.analista_solicitante || "").toLowerCase().includes(lower) || (p.nome_segurado || "").toLowerCase().includes(lower) || (p.placas_veiculos || "").toLowerCase().includes(lower) || (p.expand?.solicitante_id?.name || "").toLowerCase().includes(lower) || JSON.stringify(p.observacoes_json || {}).toLowerCase().includes(lower) || JSON.stringify(p.posicoes_json || {}).toLowerCase().includes(lower) || p.tags && p.tags.some((t) => t.toLowerCase().includes(lower)) || (p.observacoes || "").toLowerCase().includes(lower));
};
...
//# sourceMappingURL=processosService-DlaU7hSe.js.map