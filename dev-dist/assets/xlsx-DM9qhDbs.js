...
function Base64_encode(input) {
  ...
  for (var i = 0; i < input.length; ) {
    ...
    o +=
      Base64_map.charAt(e1) + Base64_map.charAt(e2) + Base64_map.charAt(e3) + Base64_map.charAt(e4)
  }
  return o
}
...
function datenum_local(v, date1904) {
  ...
  return (
    (epoch -
      (SSFdnthresh +
        /* @__PURE__ */ (v.getTimezoneOffset() - /* @__PURE__ */ SSFbasedate.getTimezoneOffset()) *
          6e4)) /
    (1440 * 60 * 1e3)
  )
}
...
var CFB = /* @__PURE__ */ (function _CFB() {
  ...
  function read_directory(dir_start, sector_list, sectors, Paths, nmfs, files, FileIndex, mini) {
    ...
    for (; i < sector.length; i += 128) {
      ...
      if (blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2) !== 0)
  ...
  function parse_zip(file, options) {
    ...
    var i = blob.length - 4
    while ((blob[i] != 80 || blob[i + 1] != 75 || blob[i + 2] != 5 || blob[i + 3] != 6) && i >= 0)
      --i
    blob.l = i + 4
    blob.l += 4
...
function datenum(v, date1904) {
  ...
  var dnthresh =
    ...
    /* @__PURE__ */ (v.getTimezoneOffset() - /* @__PURE__ */ basedate.getTimezoneOffset()) * 6e4
  return (epoch - dnthresh) / (1440 * 60 * 1e3)
}
var refdate = /* @__PURE__ */ new Date()
var dnthresh =
  /* @__PURE__ */ basedate.getTime() +
  /* @__PURE__ */ (refdate.getTimezoneOffset() - /* @__PURE__ */ basedate.getTimezoneOffset()) * 6e4
...
var good_pd_date = /* @__PURE__ */ isNaN(/* @__PURE__ */ good_pd_date_1.getFullYear())
...
function parseDate(str, fixdate) {
  ...
  if (str.indexOf('Z') > -1)
    out = /* @__PURE__ */ new Date(out.getTime() - out.getTimezoneOffset() * 60 * 1e3)
  return out
}
...
var utf8read =
  (has_buf &&
    /* @__PURE__ */ ((utf8readc(utf8corpus) == /* @__PURE__ */ utf8reada(utf8corpus) &&
      utf8readc) ||
      /* @__PURE__ */ (utf8readb(utf8corpus) == /* @__PURE__ */ utf8reada(utf8corpus) &&
        utf8readb))) ||
  utf8reada
...
function encode_range_xls(r, opts) {
  if (r.s.r == 0 && !r.s.rRel) {
    if (r.e.r == (opts.biff >= 12 ? 1048575 : opts.biff >= 8 ? 65536 : 16384) && !r.e.rRel)
      return (
        (r.s.cRel ? '' : '$') + encode_col(r.s.c) + ':' + (r.e.cRel ? '' : '$') + encode_col(r.e.c)
      )
  }
  if (r.s.c == 0 && !r.s.cRel) {
    if (r.e.c == (opts.biff >= 12 ? 16383 : 255) && !r.e.cRel)
      return (
        (r.s.rRel ? '' : '$') + encode_row(r.s.r) + ':' + (r.e.rRel ? '' : '$') + encode_row(r.e.r)
      )
  }
...
function parse_RkNumber(data) {
  ...
  data.l += 4
  var RK =
    fInt === 0 ? __double([0, 0, 0, 0, b[0] & 252, b[1], b[2], b[3]], 0) : __readInt32LE(b, 0) >> 2
...
function parse_FILETIME(blob) {
  ...
  return /* @__PURE__ */ new Date(
    ((dwHighDateTime / 1e7) * Math.pow(2, 32) + dwLowDateTime / 1e7 - 11644473600) * 1e3,
  )
    .toISOString()
    .replace(/\.000/, '')
}
function write_FILETIME(time) {
  var t =
    (typeof time == 'string' ? new Date(Date.parse(time)) : time).getTime() / 1e3 + 11644473600
...
function parse_PropertySet(blob, PIDSI) {
  ...
  for (i = 0; i != NumProps; ++i) {
    ...
    if (PIDSI) {
      ...
      if (piddsi.p === 'version')
        PropH[piddsi.n] =
          String(PropH[piddsi.n] >> 16) + '.' + ('0000' + String(PropH[piddsi.n] & 65535)).slice(-4)
...
function parse_LongRGBA(blob) {
  return [blob.read_shift(1), blob.read_shift(1), blob.read_shift(1), blob.read_shift(1)]
}
...
function parse_XTI(blob, length, opts) {
  ...
  return [blob.read_shift(w), blob.read_shift(w, 'i'), blob.read_shift(w, 'i')]
}
...
function parse_MTRSettings(blob) {
  return [blob.read_shift(4) !== 0, blob.read_shift(4) !== 0, blob.read_shift(4)]
}
...
var crypto_CreateXorArray_Method1 = /* @__PURE__ */ (function () {
  ...
  var XorMatrix = [
    ...
    11199, 17763, 35526, 1453, 2906, 5812, 11624, 23248, 885, 1770, 3540, 7080, 14160, 28320, 56640,
    55369, 41139, 20807, 41614, 21821, 43642, 17621, 28485, 56970, 44341, 19019, 38038, 14605,
    29210, 60195, 50791, 40175, 10751, 21502, 43004, 24537, 18387, 36774, 3949, 7898, 15796, 31592,
    63184, 47201, 24803, 49606, 37805, 14203, 28406, 56812, 17824, 35648, 1697, 3394, 6788, 13576,
    27152, 43601, 17539, 35078, 557, 1114, 2228, 4456, 30388, 60776, 51953, 34243, 7079, 14158,
    28316, 14128, 28256, 56512, 43425, 17251, 34502, 7597, 13105, 26210, 52420, 35241, 883, 1766,
...
function parse_PtgArea(blob, length, opts) {
  return [
    (blob[blob.l++] & 96) >> 5,
    parse_RgceArea(blob, opts.biff >= 2 && opts.biff <= 5 ? 6 : 8, opts),
  ]
}
function parse_PtgArea3d(blob, length, opts) {
  ...
  return [type, ixti, parse_RgceArea(blob, w, opts)]
}
...
function parse_PtgRef3d(blob, length, opts) {
  ...
  return [type, ixti, parse_RgceLoc(blob, 0, opts)]
}
...
function parse_PtgNameX(blob, length, opts) {
  ...
  return [(blob.read_shift(1) >>> 5) & 3, blob.read_shift(2), blob.read_shift(4)]
}
...
function csf_to_ods_formula(f) {
  return ('of:=' + f.replace(crefregex, '$1[.$2$3$4$5]').replace(/\]:\[/g, ':'))
    .replace(/;/g, '|')
    .replace(/,/g, ';')
}
...
function parse_BrtShortBool(data) {
  return [parse_XLSBShortCell(data), data.read_shift(1), 'b']
}
...
function parse_BrtShortError(data) {
  return [parse_XLSBShortCell(data), data.read_shift(1), 'e']
}
...
function parse_BrtShortIsst(data) {
  return [parse_XLSBShortCell(data), data.read_shift(4), 's']
}
...
function parse_BrtShortReal(data) {
  return [parse_XLSBShortCell(data), parse_Xnum(data), 'n']
}
...
function parse_BrtShortRk(data) {
  return [parse_XLSBShortCell(data), parse_RkNumber(data), 'n']
}
...
function parse_BrtCellRString(data) {
  return [parse_XLSBCell(data), parse_RichStr(data), 'is']
}
...
function parse_BrtShortSt(data) {
  return [parse_XLSBShortCell(data), parse_XLWideString(data), 'str']
}
...
function parse_ws_bin(data, _opts, idx, rels, wb, themes, styles) {
  ...
  if (
    !s['!ref'] &&
    (refguess.s.r < 2e6 || (ref && (ref.e.r > 0 || ref.e.c > 0 || ref.s.r > 0 || ref.s.c > 0)))
  )
...
function write_HLINKS(ba, ws, rels) {
  ws['!links'].forEach(function (l) {
    ...
    write_record(
      ba,
      494,
      write_BrtHLink(l, add_rels(rels, -1, l[1].Target.replace(/#.*$/, ''), RELS.HLINK)),
    )
  })
  delete ws['!links']
}
    ...
    switch (cell.t) {
      ...
      case 'n':
        write_biff_rec(
          ba,
          515,
          write_Number(R, C, cell.t == 'd' ? datenum(parseDate(cell.v)) : cell.v, os, opts),
        )
        break
      case 'b':
      case 'e':
      ...
      case 'str':
        if (opts.bookSST)
          write_biff_rec(
            ba,
            253,
            write_LabelSst(R, C, get_sst_id(opts.Strings, cell.v, opts.revStrings), os, opts),
          )
...
function write_biff8_global(wb, bufs, opts) {
  ...
  if (b8 && wb.vbaraw)
    write_biff_rec(A, 442, write_XLUnicodeString(_wb.CodeName || 'ThisWorkbook', opts))
  ...
  for (var i = 0; i <= wb.SheetNames.length; ++i) {
    ...
    if (decode_range(ws['!ref']).e.c > 255) {
      if (typeof console != 'undefined' && console.error)
        console.error(
          "Worksheet '" + wb.SheetNames[i] + "' extends beyond column IV (255).  Data may be lost.",
        )
    }
  }
  var o = opts || {}
  ...
  ;(entries || []).forEach(function (entry) {
    ...
    switch (type) {
      ...
      case 8:
        ...
        data[key] = parse_shallow(rtpref.data)[3]
          .map(function (x) {
            return u8str(x.data)
          })
          .join('')
...
function parse_TST_TileRowInfo(u8, type) {
  ...
  var wide_offsets =
    (((_b = (_a = pb[8]) == null ? void 0 : _a[0]) == null ? void 0 : _b.data) &&
...
function write_tile_row(tri, data, SST) {
  ...
  if (
    (((_d = (_c = tri[8]) == null ? void 0 : _c[0]) == null ? void 0 : _d.data) &&
...
function readSync(data, opts) {
  ...
  if (
    ...
    var ab = /* @__PURE__ */ new ArrayBuffer(3),
...
//# sourceMappingURL=xlsx-DM9qhDbs.js.map
