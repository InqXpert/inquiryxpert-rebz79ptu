...
var getTagColor = (tag) => {
	...
	if (t === "DOCUMENTAÇÃO PENDENTE") return "bg-amber-500 hover:bg-amber-600 text-white border-transparent";
	if (t === "AGUARDANDO TERCEIRO") return "bg-blue-500 hover:bg-blue-600 text-white border-transparent";
...
var softDeleteProcesso = async (id, executorId) => {
	const proc = await fetchProcessoById(id);
	const deletedAt = (/* @__PURE__ */ new Date()).toISOString();
	const updated = await updateProcesso(id, { deleted_at: deletedAt });
	if (executorId) await pb.collection("registros_auditoria_adm").create({
		acao: "DELETE_PROCESS",
...
var batchSoftDeleteProcessos = async (ids, executorId) => {
...
var filterByStatus = (processos, status) => {
	...
	return processos.filter((p) => {
		...
		if (status === "FINALIZADO" && (s.includes("FINALIZ") || s.includes("CONCLU")) && !s.includes("PENDENTE")) return true;
...
var searchProcessos = (processos, search) => {
	...
	return processos.filter((p) => (p.id || "").toLowerCase().includes(lower) || (p.numero_controle || "").toLowerCase().includes(lower) || (p.data_entrada || "").toLowerCase().includes(lower) || (p.analista_solicitante || "").toLowerCase().includes(lower) || (p.nome_segurado || "").toLowerCase().includes(lower) || (p.placas_veiculos || "").toLowerCase().includes(lower) || (p.expand?.solicitante_id?.name || "").toLowerCase().includes(lower) || JSON.stringify(p.observacoes_json || {}).toLowerCase().includes(lower) || JSON.stringify(p.posicoes_json || {}).toLowerCase().includes(lower) || Array.isArray(p.tags) && p.tags.some((t) => t.toLowerCase().includes(lower)) || (p.observacoes || "").toLowerCase().includes(lower));
};
...
var fetchProcessoById = async (id) => {
	return await pb.collection("processos_operacionais").getOne(id, { expand: "agente_id,supervisor_id,cliente_id,seguradora_id,natureza_sinistro_id,tipo_investigacao_id,solicitante_id" });
};
...
var generateNumeroControle = async (cia, natureza) => {
	...
	let nn = "00";
	try {
		const clientes = await pb.collection("clientes_contratos").getList(1, 1, { filter: `razao_social = '${cia.replace(/'/g, "\\'")}'` });
		if (clientes.items.length > 0 && clientes.items[0].codigo) cc = clientes.items[0].codigo;
		else throw new Error(`Cliente (${cia}) não possui código configurado.`);
	} catch (e) {
		if (e.message.includes("não possui")) throw e;
		throw new Error(`Erro ao buscar cliente para geração do número: ${e.message}`);
	}
	try {
		const naturezas = await pb.collection("naturezas_sinistro").getList(1, 1, { filter: `nome = '${natureza.replace(/'/g, "\\'")}'` });
		if (naturezas.items.length > 0 && naturezas.items[0].codigo) nn = naturezas.items[0].codigo;
		else throw new Error(`Natureza de sinistro (${natureza}) não possui código configurado.`);
	} catch (e) {
		if (e.message.includes("não possui")) throw e;
...
//#endregion
export { updateProcesso as C, transcribeAudio as S, markProcessosAsRead as _, createProcesso as a, softDeleteProcesso as b, fetchProcessoById as c, filterByDate as d, filterByFavorites as f, getTagColor as g, generateNumeroControle as h, createAuditLog as i, fetchProcessos as l, filterByStatus as m, calculateDayColor as n, deleteProcesso as o, filterByPriority as p, calculateTags as r, fetchFavoritos as s, batchSoftDeleteProcessos as t, fetchProcessosAgente as u, searchByNumero as v, validateDuplicidade as w, toggleProcessoFavorite as x, searchProcessos as y };

//# sourceMappingURL=processosService-Buql7rLA.js.map