...
import { t as differenceInDays } from "./differenceInDays-BH-BDO1i.js";
...
var lastFetch = 0;
var TTL = 1e4;
var fetchAlertas = async (force = false) => {
	const now = Date.now();
	if (!force && fetchPromise && now - lastFetch < TTL) return fetchPromise;
	if (force && fetchPromise && now - lastFetch < 1e3) return fetchPromise;
	lastFetch = now;
	fetchPromise = pb.collection("processos_operacionais").getFullList({
		filter: "status != 'FINALIZADO' && (data_conclusao = '' || status = 'concluido_pendente_documentos')",
...
var calculateAlertLevel = (processos, userId, userRole) => {
	...
	for (const p of processos) {
		...
		if (st === "CONCLUIDO_PENDENTE_DOCUMENTOS" || st === "PENDENTE_DOCUMENTOS") {
			const diffPendencia = differenceInDays(today, p.data_entrada_pendencia ? new Date(p.data_entrada_pendencia) : new Date(p.updated));
			if (diffPendencia > 3) alertas.push({
				...baseAlerta,
			...
			else if (diffPendencia >= 1) alertas.push({
				...baseAlerta,
		...
		if (p.prioridade === "alta") alertas.push({
			...baseAlerta,
...
function useAlertas() {
	...
	const loadData = (0, import_react.useCallback)(async (isInitial = false, force = false) => {
		if (!user) return;
		try {
			if (isInitial) setLoading(true);
			const calcAlertas = calculateAlertLevel(await fetchAlertas(force), user.id, user.role);
...
//#endregion
export { useAlertas as t };

//# sourceMappingURL=useAlertas-B6QcR5n5.js.map